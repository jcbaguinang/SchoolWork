...
def extract_words(text):
    ...
    for i in range(len(text)):
        if text[i] in ascii_letters:
            ...
            string = ''
        if len(string) >= 1 and i == len(text) -1:
            word_list.append(string)
    return word_list
...
def analyze_tweet_sentiment(tweet):
    ...
    for w in tweet_words(tweet): 
        if w in word_sentiments.keys():
            sum += word_sentiments[w]
            len += 1
    if len == 0:
        return make_sentiment(None)
    return make_sentiment(sum / len)
...
def find_centroid(polygon):
    ...
    """
    def used_in_all(polygon, i):
        return latitude(polygon[i]) * longitude(polygon[i + 1])\
            - latitude(polygon[i + 1]) * longitude(polygon[i])

    total_area, total_x, total_y = 0, 0, 0
    for i in range(len(polygon) - 1):
        total_area += used_in_all(polygon, i)
        this_x, next_x = latitude(polygon[i]), latitude(polygon[i + 1])
        this_y, next_y = longitude(polygon[i]), longitude(polygon[i + 1])
        total_x += (this_x + next_x) * used_in_all(polygon, i)
        total_y += (this_y + next_y) * used_in_all(polygon, i)
    total_area *= .5
    if total_area == 0:
        return latitude(polygon[0]), longitude(polygon[0]), 0

    return 1 / (6 * total_area) * total_x, 1 / (6 * total_area) * total_y, abs(total_area)
...
def average_sentiments(tweets_by_state):
    ...
    for key in tweets_by_state:
        total_sentiment = 0
        num_sentiment_tweets = 0
        for value in tweets_by_state[key]:
            if has_sentiment(analyze_tweet_sentiment(value)):
                num_sentiment_tweets += 1
                total_sentiment += sentiment_value(analyze_tweet_sentiment(value))
        if num_sentiment_tweets != 0:
            averaged_state_sentiments[key] = total_sentiment / num_sentiment_tweets
