...
(define (merge comp list1 list2)
  (cond ((and (null? list1) (null? list2)) nil)
        ((null? list1) (append (list(car list2)) (merge comp list1 (cdr list2))))
        ((null? list2) (append (list(car list1)) (merge comp (cdr list1) list2)))
...
(define (add_list proc items)
  (cond ((null? items) nil)
        (else (cons (+ proc (car items))
            (add_list proc (cdr items))))))
(define (flatten lst)
    (cond ((null? lst) nil)
    ((number? (car lst)) (cons (car lst) (flatten (cdr lst))))
    (else (append (flatten (car lst)) (flatten (cdr lst))))))
(define (tree-sums tree)
  (cond ((null? (children tree)) (list (entry tree)))
        (else (add_list (entry tree) (flatten (map tree-sums (children tree)))))))
