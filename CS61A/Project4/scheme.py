...
def apply_primitive(procedure, args, env):
    ...
    def traverse_list(args):
        if args is nil:
            ...
            return traverse_list(args.second)
    try:
        traverse_list(args)
        ...
        raise SchemeError("An error was caught!")
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target.first): 
            ...
            raise SchemeError("This is not a proper variable")
    else:
    ...
    def new_check(x):
        ...
        while x is not nil:
            if not isinstance(x, Pair):
                return False
            if scheme_symbolp(x.first):
                if x.first not in symbols:
                    symbols.append(x.first)
                else:
                    return False
            else:
                return False
...
def scheme_optimized_eval(expr, env):
    ...
    while True:
        ...
        if (scheme_symbolp(first) # first might be unhashable
            ...
            procedure = scheme_eval(first, env)
            args = rest.map(lambda operand: scheme_eval(operand, env))
            if isinstance(expr, PrimitiveProcedure):
                return apply_primitive(procedure, args, env)
            elif isinstance(expr, LambdaProcedure):
                "*** YOUR CODE HERE ***"
                expr, env = procedure.body, env.make_call_frame(procedure.formals, args)
            elif isinstance(expr, MuProcedure):
                "*** YOUR CODE HERE ***"
                expr, env = procedure.body, env.make_call_frame(procedure.formals, args)
            else:
                raise SchemeError("Cannot call {0}".format(str(procedure)))
