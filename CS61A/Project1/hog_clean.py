...
def play(strategy0, strategy1, goal=GOAL_SCORE):
    ...
    while score < GOAL_SCORE and opponent_score < GOAL_SCORE:
        if who == 0:
            score = score + take_turn(strategy0(score, opponent_score), \
                        opponent_score, select_dice(score, opponent_score))
        else:
            opponent_score = opponent_score + take_turn(strategy1(opponent_score, score), \
                                score, select_dice(opponent_score, score))
        if score == opponent_score // 2 and opponent_score % 2 == 0 \
        or opponent_score == score // 2 and score % 2 == 0:
...
def swap_strategy(score, opponent_score):
    ...
    both = bacon_number + score
    if both == opponent_score//2 and opponent_score%2==0:
        return 0
    elif both//2 == opponent_score and both % 2 == 0:
        return BASELINE_NUM_ROLLS
...
def final_strategy(score, opponent_score):
    """Write a brief description of your final strategy.
        First, if you can win immediately, you do it
        Then, you attempt to swap if you can for sure do it
        Avoid swapping if possible
        Then, attempt to get half of the opponent's score to swap 
        Finally, roll safely if you're up and take risks if you're down
    """ 
    hog_wild=score+opponent_score%7
    bacon_number = 1 + max((opponent_score//10), (opponent_score % 10))
    both = score + bacon_number
    oppdiv = opponent_score//2
    oppmod = opponent_score % 2

    """If the bacon number wins I use it unless it results in a bad swap
    """
    if both >= 100 and (both//2 != opponent_score or both % 2 != 0):
        return 0

    """ Attempt to swap
    """
    if both == opponent_score//2 and oppmod == 0:
        return 0
    if score > 4:
        if(score + 1) == oppdiv and oppmod == 0:
            return 10  

    """ Avoid swap
    """
    if both//2 == opponent_score and both % 2 == 0:
        return BASELINE_NUM_ROLLS
    if score > 15:
    ...
    Force Hog Wild
    """
    if (both + opponent_score) % 7 == 0 and bacon_number>4:
        return 0

    """
    Use a good bacon number
    """
    if bacon_number >= 8:
        return 0

    """Attempt to get half of opponent's score
    """
    if opponent_score>17 and oppdiv - score>1:
        n = 1
        count = 0
        if hog_wild==0:
            while count < 10 and oppdiv - score>n:
                n = n + 3
                count += 1
            return count
        else:
